# Referensi API — Industrial Protocols Go

> Ikhtisar proyek lihat [README](../README.md) · Versi Inggris: [API.en.md](API.en.md)
...
```go
// kernel/protocol.go — harus diimplementasikan oleh setiap modul protokol
...
// kernel/transport/transport.go — saluran transport
...
| Middleware | Konstruktor | Deskripsi |
|--------|------|------|
| `Timeout` | `Timeout(d time.Duration)` | Kontrol timeout context |
| `Retry` | `Retry(max int, backoff BackoffFunc)` | Tiga jenis backoff: linear / exponential / jitter |
| `CircuitBreaker` | `NewCircuitBreaker(threshold int, cooldown time.Duration)` | Circuit breaker tiga keadaan: closed → open → half-open |
| `Logger` | `Logger(logger *log.Logger)` | Log permintaan terstruktur |

## Panduan Penggunaan

### Instalasi

Pustaka inti Kernel adalah dependensi wajib; modul protokol diimpor sesuai kebutuhan. Semua modul memakai versi seragam `v1.1.2`.
...
> **Alamat pkg.go.dev:** `https://pkg.go.dev/github.com/erikwang2013/industrial-protocols-go/` + jalur submodul (mis. `/kernel`, `/protocols/ethernet/modbus`)

### Baca/Tulis Modbus TCP
...
### Menggunakan ConnectionManager + Connection Pool

```go
mgr := connection.NewManager(&connection.PooledStrategy{})

// Registrasi perangkat (dari kode)
...
// Atau muat dari konfigurasi YAML
repo, _ := config.Load("devices.yaml")
for _, cfg := range repo.All() {
    mgr.Register(cfg)
}

// Ambil koneksi → gunakan → kembalikan
...
### Konversi Gateway Antar-Protokol
...
Mengimplementasikan protokol baru hanya memerlukan dua file:
