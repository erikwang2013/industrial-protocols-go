# API রেফারেন্স — Industrial Protocols Go

> প্রকল্প ওভারভিউ: [README](../../README.md) · ইংরেজি সংস্করণ: [API.en.md](../../API.en.md)
...
| মিডলওয়্যার | কনস্ট্রাক্টর | বিবরণ |
|--------|------|------|
| `Timeout` | `Timeout(d time.Duration)` | context টাইমআউট নিয়ন্ত্রণ |
| `Retry` | `Retry(max int, backoff BackoffFunc)` | linear / exponential / jitter — তিন ধরনের ব্যাকঅফ |
| `CircuitBreaker` | `NewCircuitBreaker(threshold int, cooldown time.Duration)` | closed → open → half-open তিন অবস্থার সার্কিট ব্রেকার |
| `Logger` | `Logger(logger *log.Logger)` | স্ট্রাকচার্ড রিকোয়েস্ট লগ |

## ব্যবহার নির্দেশিকা

### ইনস্টলেশন

Kernel কোর লাইব্রেরি একটি আবশ্যক নির্ভরতা; প্রোটোকল মডিউলগুলো প্রয়োজন অনুযায়ী ইমপোর্ট করা হয়। সব মডিউলের অভিন্ন ভার্সন `v1.1.2`।
...
> **pkg.go.dev ঠিকানা:** `https://pkg.go.dev/github.com/erikwang2013/industrial-protocols-go/` + সাবমডিউল পাথ (যেমন `/kernel`, `/protocols/ethernet/modbus`)

### Modbus TCP পড়া/লেখা
...
### ConnectionManager + কানেকশন পুল ব্যবহার
...
### প্রোটোকলের মধ্যে গেটওয়ে রূপান্তর
...
একটি নতুন প্রোটোকল বাস্তবায়ন করতে শুধুমাত্র দুটি ফাইল লাগে:
