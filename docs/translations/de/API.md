# API-Referenz — Industrial Protocols Go

> Projektübersicht siehe [README](../../README.md) · Englische Version: [API.en.md](../API.en.md)
...
| Middleware | Konstruktor | Beschreibung |
|--------|------|------|
| `Timeout` | `Timeout(d time.Duration)` | Context-Timeout-Steuerung |
| `Retry` | `Retry(max int, backoff BackoffFunc)` | drei Backoff-Strategien: linear / exponentiell / jitter |
| `CircuitBreaker` | `NewCircuitBreaker(threshold int, cooldown time.Duration)` | Drei-Zustands-Unterbrechung: closed → open → half-open |
| `Logger` | `Logger(logger *log.Logger)` | strukturierte Request-Logs |

## Verwendungsanleitung

### Installation

Die Kernel-Kernbibliothek ist eine zwingende Abhängigkeit; Protokollmodule werden bei Bedarf eingebunden. Alle Module verwenden die einheitliche Version `v1.1.2`.
...
> **pkg.go.dev-Adressen:** `https://pkg.go.dev/github.com/erikwang2013/industrial-protocols-go/` + Untermodul-Pfad (z. B. `/kernel`, `/protocols/ethernet/modbus`)

### Modbus-TCP Lesen/Schreiben
...
### ConnectionManager + Verbindungspool verwenden
...
### Protokollübergreifende Gateway-Konvertierung
...
Um ein neues Protokoll zu implementieren, genügen zwei Dateien:
