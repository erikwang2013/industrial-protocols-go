# APIリファレンス — Industrial Protocols Go

> プロジェクト概要は [README](README.md) · 英語版：[API.en.md](../../API.en.md)
...
| ミドルウェア | コンストラクタ | 説明 |
|--------|------|------|
| `Timeout` | `Timeout(d time.Duration)` | contextタイムアウト制御 |
| `Retry` | `Retry(max int, backoff BackoffFunc)` | linear / exponential / jitter の3種類のバックオフ |
| `CircuitBreaker` | `NewCircuitBreaker(threshold int, cooldown time.Duration)` | closed → open → half-open の3状態サーキットブレーカー |
| `Logger` | `Logger(logger *log.Logger)` | 構造化リクエストログ |

## 使用ガイド

### インストール

Kernelコアライブラリは必須の依存関係で、プロトコルモジュールは必要に応じて導入します。全モジュールのバージョンは統一で `v1.1.2` です。
...
> **pkg.go.dev アドレス：** `https://pkg.go.dev/github.com/erikwang2013/industrial-protocols-go/` + サブモジュールパス（例：`/kernel`、`/protocols/ethernet/modbus`）
...
### ConnectionManager + コネクションプールの使用
